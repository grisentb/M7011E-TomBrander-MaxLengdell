...
export default class Dashboard extends React.Component {
  ...
  update() {
    ...
    if (this.state.prosumer) {
      ...
      axios.get('https://130.240.200.39:4000/consumer/prosumer', { params: { _id: this.state.prosumer._id } }).then(resp => {
        consumers = resp.data;
  ...
  render() {
    ...
    const changeRatio = async e => {
      if (e.key === 'Enter') {
        ...
        axios.post('https://130.240.200.39:4000/home/ratio', { _id: this.state.prosumer._id, value: value }).then(resp => {
          if (resp.data === false) {
    ...
    const changeCapacity = async e => {
      if (e.key === 'Enter') {
        ...
        axios.post('https://130.240.200.39:4000/home/capacity', { _id: this.state.prosumer._id, value: e.target.value }).then(resp => {
          e.target.value = null;
        });
      }
    }
    ...
    const newHouse = async e => {
      await axios.post('https://130.240.200.39:4000/consumer/consumption', { _id: this.state.prosumer._id });
    ...
    const handleLogout = () => {
      ...
      axios.post('https://130.240.200.39:4000/logout',{user: this.user}).then(resp => {
